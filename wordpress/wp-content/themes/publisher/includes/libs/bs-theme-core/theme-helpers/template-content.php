...
if ( ! function_exists( 'publisher_cats_badge_code' ) ) {
	...
	function publisher_cats_badge_code( $cats_count = 1, $sep = '', $show_format = FALSE, $echo = TRUE, $class = '', $show_cats = TRUE ) {
		...
		if ( ! bf_taxonomy_supports_post_type( 'category', get_post_type() ) ) {
			return;
		}
		...
		if ( $show_cats ) {
			if ( $cats_count == - 1 ) {
				...
				foreach ( $cats as $cat_id => $cat ) {
					...
					$cat_code[] = '<span class="term-badge term-' . $cat->term_id . '"><a href="' . esc_url( get_category_link( $cat ) ) . '">' . esc_html( $cat->name ) . '</a></span>';
				}
