		...
		<div class="post-share <?php echo esc_attr( $args['class'] ); ?>">
			...
			if ( $args['type'] === 'single' ) {
				...
				<span class="share-handler post-share-btn">
					...
					<?php if ( $total_count = array_sum( $count_labels ) ) { ?>
			...
			if ( $args['type'] === 'single' && $args['show_comments'] && comments_open() ) {
				...
				printf( '<a href="%1$s" class="post-share-btn post-share-btn-comments" title="%2$s" ' . publisher_get_attr( 'post-meta-comments' ) . '><i class="fa fa-comments" aria-hidden="true"></i> <b class="number">%3$s</b></a>',
					esc_url( $link ),
			...
			if ( $args['type'] === 'single' && $args['show_views'] && function_exists( 'The_Better_Views' ) ) {
				The_Better_Views(
					TRUE,
					'<span ' . publisher_get_attr( 'post-meta-views', 'post-share-btn post-share-btn-views' ) . ' data-bpv-post="' . get_the_ID() . '"><i class="fa fa-eye"></i> ',
					'</span>',
					'show',
					'<b class="number">%VIEW_COUNT%</b>'
				);
			}
	...
	function publisher_layout_option_list( $default = FALSE ) {
		...
		if ( $default ) {
			...
			$option['1-col']   = array(
				...
				'label' => __( 'Full Width 1', 'publisher' ),
			);
			$option['3-col-0'] = array(
				'img'   => PUBLISHER_THEME_URI . 'images/options/layout-3-col-0.png',
				'label' => __( 'Full Width 2', 'publisher' ),
			);
		}

		$option['2-col-right'] = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/layout-2-col-right.png',
			'label' => __( '2 Col - 1', 'publisher' ),
			'class' => 'bf-flip-img-rtl',
		);
		$option['2-col-left']  = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/layout-2-col-left.png',
			'label' => __( '2 Col - 2', 'publisher' ),
			'class' => 'bf-flip-img-rtl',
		);

		$option['3-col-1'] = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/layout-3-col-1.png',
			'label' => __( '3 Col - 1', 'publisher' ),
			'class' => 'bf-flip-img-rtl',
		);
		$option['3-col-2'] = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/layout-3-col-2.png',
			'label' => __( '3 Col - 2', 'publisher' ),
			'class' => 'bf-flip-img-rtl',
		);
		$option['3-col-3'] = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/layout-3-col-3.png',
			'label' => __( '3 Col - 3', 'publisher' ),
			'class' => 'bf-flip-img-rtl',
		);
		$option['3-col-4'] = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/layout-3-col-4.png',
			'label' => __( '3 Col - 4', 'publisher' ),
			'class' => 'bf-flip-img-rtl',
		);
		$option['3-col-5'] = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/layout-3-col-5.png',
			'label' => __( '3 Col - 5', 'publisher' ),
			'class' => 'bf-flip-img-rtl',
		);
		$option['3-col-6'] = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/layout-3-col-6.png',
			'label' => __( '3 Col - 6', 'publisher' ),
			'class' => 'bf-flip-img-rtl',
		);
...
if ( ! function_exists( 'publisher_get_show_page_listing_excerpt' ) ) {
	...
	function publisher_get_show_page_listing_excerpt( $wp_query = NULL ) {
		...
		elseif ( $wp_query->is_author ) {
			$excerpt = bf_get_user_meta( 'page_listing_excerpt', $wp_query->get_queried_object() );
	...
	function publisher_header_style_option_list( $default = FALSE, $disable = TRUE ) {
		...
		if ( $disable ) {
			$option['disable'] = array(
				'img'   => PUBLISHER_THEME_URI . 'images/options/header-disable.png',
...
if ( ! function_exists( 'publisher_footer_style_option_list' ) ) {
	...
	function publisher_footer_style_option_list( $default = FALSE, $disable = TRUE ) {
		...
		if ( $default ) {
			$option['default'] = array(
				'img'   => PUBLISHER_THEME_URI . 'images/options/footer-default.png',
				'label' => __( '-- Default --', 'publisher' ),
			);
		}
		...
		if ( $disable ) {
			$option['disable'] = array(
				'img'   => PUBLISHER_THEME_URI . 'images/options/footer-disable.png',
...
if ( ! function_exists( 'publisher_topposts_option_list' ) ) {
	...
	function publisher_topposts_option_list( $default = FALSE ) {
		...
		$option['style-17'] = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/cat-slider-style-17.png',
			'label' => __( 'Style 17', 'publisher' ),
			'class' => 'bf-flip-img-rtl',
		);
		$option['style-18'] = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/cat-slider-style-18.png',
			'label' => __( 'Style 18', 'publisher' ),
			'class' => 'bf-flip-img-rtl',
		);
		$option['style-19'] = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/cat-slider-style-19.png',
			'label' => __( 'Style 19', 'publisher' ),
			'class' => 'bf-flip-img-rtl',
		);
		$option['style-20'] = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/cat-slider-style-20.png',
			'label' => __( 'Style 20', 'publisher' ),
			'class' => 'bf-flip-img-rtl',
		);
		$option['style-21'] = array(
			'img'   => PUBLISHER_THEME_URI . 'images/options/cat-slider-style-21.png',
		...
		<div class="post-meta">
			...
			if ( $show_date ) {
				...
				<span class="time"><time class="post-published updated"
				                         datetime="<?php echo mysql2date( DATE_W3C, $post->post_date, FALSE ); ?>"><?php the_time( publisher_translation_get( 'comment_time' ) ); ?></time></span>
				<?php
			}
